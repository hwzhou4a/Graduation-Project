...
cat("   > Drawing VST PCA...\n")
draw_custom_pca(vst_mat_subset, "PCA based on gene expression (VST)", "Refined_PCA_VST.pdf")
...
# ==============================================================================
# Refined Part 6: Enrichment Analysis (GO & KEGG) with Specific Thresholds
# ==============================================================================
cat("\n[Part 6] Refined Enrichment Analysis...\n")

library(enrichplot)

# 1. Prepare Gene IDs (Symbol -> Entrez)
# We use the significant genes identified in Part 4
gene_symbols <- rownames(sig_genes_df)
ids <- bitr(gene_symbols, fromType="SYMBOL", toType="ENTREZID", OrgDb=org.Hs.eg.db)

if(nrow(ids) > 0) {
  gene_list <- ids$ENTREZID
  
  # ----------------------------------------------------------------------------
  # A. GO Enrichment (BP, MF, CC combined)
  # Thresholds: pvalue < 0.05 AND qvalue < 0.05 (Strict)
  # ----------------------------------------------------------------------------
  cat("   > Running GO Analysis (p<0.05, q<0.05)...\n")
  
  ego <- enrichGO(gene          = gene_list,
                  OrgDb         = org.Hs.eg.db,
                  ont           = "ALL",      # Analyze Biological Process, Molecular Function, and Cellular Component
                  pAdjustMethod = "BH",
                  pvalueCutoff  = 0.05,
                  qvalueCutoff  = 0.05,       # Strict threshold as requested
                  readable      = TRUE)       # Convert Entrez back to Symbol in results
  
  if(!is.null(ego) && nrow(ego) > 0) {
    # 1. Generate Table
    # Explicitly filter again to ensure the dataframe matches criteria (just in case)
    df_go <- as.data.frame(ego) %>% filter(pvalue < 0.05 & qvalue < 0.05)
    write.csv(df_go, "Enrichment_Table_GO.csv", row.names = FALSE)
    
    # 2. Generate Bubble Plot (Dotplot)
    # split="ONTOLOGY" separates BP, MF, CC into sub-plots for clarity
    p_go_dot <- dotplot(ego, showCategory=10, split="ONTOLOGY") + 
      facet_grid(ONTOLOGY~., scale="free") +
      ggtitle("GO Enrichment Bubble Plot") +
      theme(plot.title = element_text(hjust = 0.5, face = "bold"))
    
    ggsave("Enrichment_BubblePlot_GO.pdf", p_go_dot, width = 10, height = 12)
    cat("     [OK] GO table and bubble plot saved.\n")
  } else {
    cat("     [!] No GO terms met the strict thresholds (p<0.05, q<0.05).\n")
  }
  
  # ----------------------------------------------------------------------------
  # B. KEGG Enrichment
  # Thresholds: pvalue < 0.05 AND qvalue < 0.2 (Looser q-value)
  # ----------------------------------------------------------------------------
  cat("   > Running KEGG Analysis (p<0.05, q<0.2)...\n")
  
  ekegg <- enrichKEGG(gene         = gene_list,
                      organism     = 'hsa',    # Homo sapiens
                      pvalueCutoff = 0.05,
                      qvalueCutoff = 0.2)      # Looser threshold as requested
  ...
  if(!is.null(ekegg) && nrow(ekegg) > 0) {
    ...
    # 1. Generate Table
    df_kegg <- as.data.frame(ekegg) %>% filter(pvalue < 0.05 & qvalue < 0.2)
    write.csv(df_kegg, "Enrichment_Table_KEGG.csv", row.names = FALSE)
    
    # 2. Generate Bubble Plot (Dotplot)
    p_kegg_dot <- dotplot(ekegg, showCategory=20) + 
      ggtitle("KEGG Pathway Enrichment Bubble Plot") +
      theme(plot.title = element_text(hjust = 0.5, face = "bold"))
    
    ggsave("Enrichment_BubblePlot_KEGG.pdf", p_kegg_dot, width = 8, height = 8)
    cat("     [OK] KEGG table and bubble plot saved.\n")
  } else {
    cat("     [!] No KEGG pathways met the thresholds (p<0.05, q<0.2).\n")
  }
